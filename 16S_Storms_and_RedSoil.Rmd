...
```{r allsample_pcoa, message=FALSE}
ordu = ordinate(psCLR, "PCoA", "euclidean")
plot_ordination(psCLR, ordu, color="Type", shape = "Month")+theme_bw() +scale_color_brewer(palette= "Paired")+ geom_point(size=4) +  theme(text = element_text(size=24))
```
...
p<-plot_ordination(psCLRfield, ordu, color="Time", shape = "Month")+theme_bw() +scale_color_manual(values=c("#CC223B","#004488", "#CC223B", "#88CCEE", "#004488","#E6D02E" ,"#88CCEE"))+ geom_point(size=4) +  theme(text = element_text(size=24))
...
meta <- metatable2[row.names(metatable2) %in% row.names(OTUField),]

set.seed(1)
adonis2(vegdist(OTUField , method = "euclidean") ~ Month, data = meta)
```


June and October separate PCoA: 
```{r juneField_pcoa}
psCLRfieldjune <-  subset_samples(psCLRfield, Month == "June") 
ordu = ordinate(psCLRfieldjune, "PCoA", "euclidean")
pJune<-plot_ordination(psCLRfieldjune, ordu, color="Time")+theme_bw() +scale_color_manual(values=c("#004488", "#CC223B", "#88CCEE"), name = "")+ geom_point(size=4) +  theme(text = element_text(size=18)) + theme(legend.position="bottom") +ggtitle("A. June") + theme(legend.title = element_blank())


psCLRfieldOct <-  subset_samples(psCLR, Month == "Oct" & Type == "Field") 
ordu = ordinate(psCLRfieldOct, "PCoA", "euclidean")

pOct<-plot_ordination(psCLRfieldOct, ordu, color="Time")+theme_bw() +scale_color_manual(values=c("#004488", "#CC223B","#E6D02E" ,"#88CCEE"), limits=c("28-Sep", "1-Oct", "3-Oct", "8-Oct"), name = "")+ geom_point(size=4) +  theme(text = element_text(size=18))+ theme(legend.position="bottom") + ggtitle("B. October")


grid.arrange(pJune, pOct, ncol=2)
#ggsave("JuneOct_sepPCoA.pdf", arrangeGrob(pJune, pOct, ncol=2), width = 10, height = 6)
```
...
June & October Mesocosms plotted separately: 
```{r JuneOct_AB_meso_pcoa}
psCLRmesoJ <-  subset_samples(psCLR, Type == "Meso" & Time != "t6" & Month =="June" & Treat != "F")  
ordu = ordinate(psCLRmesoJ, "PCoA", "euclidean")
pMesoJune<-plot_ordination(psCLRmesoJ, ordu, color="Time", shape = "Treat")+theme_bw() +scale_color_jcolors(palette="pal7", name = "Time")+ geom_point(size=4) +  theme(text = element_text(size=16))+ ggtitle("A. June") + scale_shape_manual(values = c(19,17), labels= c("Control", "Red Soil"), name = "Treatment")

psCLRmesoO <-  subset_samples(psCLR, Type == "Meso" & Time != "t6" & Month =="Oct" &Treat != "F") 
ordu = ordinate(psCLRmesoO, "PCoA", "euclidean")
pMesoOct<-plot_ordination(psCLRmesoO, ordu, color="Time", shape = "Treat")+theme_bw() +scale_color_jcolors(palette="pal7", name = "Time")+ geom_point(size=4) + scale_shape_manual(values = c(19,17), labels= c("Control", "Red Soil"), name = "Treatment") + theme(text = element_text(size=16)) +ggtitle("B. October")

ggarrange(pMesoJune, pMesoOct,   common.legend = TRUE, legend = "bottom")
#ggsave("MESO_PCoA.png", width = 8, height = 5)
```
...
```{r}
sigplot<- ggplot(sigtabJ12wSoil, aes(x=D1, y=log2FoldChange, color = D1)) + geom_jitter(size=1.5, alpha = 0.7) + theme(legend.title=element_blank()) + theme_bw() + ggtitle("June 16 v. June 13 ") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))
